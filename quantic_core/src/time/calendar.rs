...
pub trait Calendar {
    ...
    fn is_business_day(&self, date: &NaiveDate) -> bool {
        match self.holiday(date) {
            Some(_) => false,
            None => !self.is_weekend(date),
        }
    }
...
pub trait WesternCalendar {
    fn easter_monday(&self, year: i32) -> u32 {
        assert!(year > 1900);
        assert!(year < 2200);

        const EASTER_MONDAY: &'static [u32] = &[
        ...
        EASTER_MONDAY[year as usize - 1901]
    }
}

pub trait OrthodoxCalendar {
    fn easter_monday(year: i32) -> u32 {
        assert!(year > 1900);
        assert!(year < 2200);
...
mod tests {
    ...
    struct TestCalendar;
    impl WesternCalendar for TestCalendar { }
    ...
    fn test_holiday() {
        ...
        assert!(calendar.holiday(&NaiveDate::from_ymd(2019, 1, 1)).is_some());
        assert!(calendar.holiday(&NaiveDate::from_ymd(2019, 4, 18)).is_some());
        assert!(calendar.holiday(&NaiveDate::from_ymd(2019, 4, 19)).is_some());
        assert!(calendar.holiday(&NaiveDate::from_ymd(2019, 9, 1)).is_none());
    }
}